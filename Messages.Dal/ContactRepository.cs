...
public class ContactRepository(Context context) : IContactRepository
{
    ...
    //public async Task GetCustomerByTransaction()
    //{
    //    var result = await context.Users.Where(n => context.Messages.Count(i =>
    //        i.SenderId == n.Id) > 42).ToListAsync();
    //    //add condition in count by date and type transaction and role
    //}

    //public void GetCustomerByTransactionSum()
    //{
    //    var sum = context.Users.Where(n =>

    //        context.Messages.Where(m => m.SenderId == n.Id).Sum(r => r.Id) > 120);
    //}
